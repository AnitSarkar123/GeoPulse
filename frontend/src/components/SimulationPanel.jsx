...
/**
 * SimulationPanel — Geopolitical "what if" scenario analyzer.
 * Uses the /api/simulate endpoint with LLM-powered predictions.
 */
...
const presets = [
  ...
  "If NATO deploys to Eastern Europe, what are the economic consequences?",
  "If a major earthquake hits Tokyo, what's the global market impact?",
  "If oil prices spike to $200/barrel, what happens to geopolitics?",
  "If US-China trade war escalates fully, what's the impact on developing nations?",
];
...
export default function SimulationPanel({ isOpen, onClose }) {
  ...
  const runSimulationHandler = async (text) => {
    ...
    try {
      const simulationResult = await runSimulation(input);
          ...
          <div className="flex-1 overflow-y-auto p-6 space-y-6">
            ...
            <div className="space-y-3">
              ...
              <div className="flex gap-2">
                <input
                  ...
                  onKeyDown={(e) => e.key === 'Enter' && runSimulationHandler()}
                  placeholder="What if..."
              ...
              <div className="flex flex-wrap gap-2">
                {presets.map((p, i) => (
                  <button
                    key={i}
                    onClick={() => { setScenario(p); runSimulationHandler(p); }}
                    disabled={loading}
                    className="text-[10px] px-3 py-1.5 glass-light rounded-md hover:bg-[var(--bg-elevated)] text-[var(--text-secondary)] hover:text-white transition-colors disabled:opacity-50"
            ...
            {error && (
              <motion.div
                ...
                className="glass-light rounded-lg p-4 border border-red-500/30 bg-red-500/5"
              >
