...
import { chatWithIntel } from '../services/api';

/**
 * ChatBot — AI Intelligence Assistant
 * Connects to the /api/chat backend endpoint for real geopolitical analysis.
 */
export default function ChatBot() {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      const response = await chatWithIntel(userMessage, messages);
          ...
          >
            ...
            <div className="px-4 py-3 border-b border-[var(--border-default)] flex items-center justify-between flex-shrink-0">
              <div className="flex items-center gap-2">
                ...
                <span className="text-[9px] font-mono px-1.5 py-0.5 rounded-sm bg-green-500/20 text-green-400">Live</span>
              </div>
              <button
            ...
            <div className="px-4 py-3 border-t border-[var(--border-default)] flex-shrink-0">
              <div className="flex items-center gap-2">
                <input
                  ...
                  disabled={loading}
                  className="flex-1 bg-[var(--bg-elevated)] rounded-md px-3 py-2.5 text-sm border border-[var(--border-default)] outline-none focus:border-[var(--cat-political)] transition-colors text-[var(--text-primary)] placeholder:text-[var(--text-muted)] disabled:opacity-50"
