...
const GEOPOLITICS_URL = `${BASE_URL}/geopolitics`;
const SIMULATE_URL = `${BASE_URL}/simulate`;
const COUNTRY_INTEL_URL = `${BASE_URL}/country-intel`;
...
export const fetchGeopoliticsData = async () => {
  const res = await axios.get(GEOPOLITICS_URL, { timeout: 15000 });
 ...
 * @param {string} scenario - The hypothetical scenario
 * @returns {Promise<Object>} Simulation results with probability, severity, chain reactions, etc.
 */
export const runSimulation = async (scenario) => {
  const res = await axios.post(SIMULATE_URL, { scenario }, { timeout: 30000 });

  if (!res.data.success) {
    throw new Error(res.data.error || 'Simulation failed');
  }
...
/**
 * Get AI-generated intelligence for a country
 * @param {string} country - Country name
 * @returns {Promise<Object>} Country intelligence data
 */
export const getCountryIntel = async (country) => {
  const res = await axios.post(COUNTRY_INTEL_URL, { country }, { timeout: 30000 });

  if (!res.data.success) {
    throw new Error(res.data.error || 'Country intel failed');
  }

  return res.data;
};
/**
 * Chat with AI intelligence assistant
 * @param {string} message - User message
 * @param {Array} conversationHistory - Previous messages
 * @returns {Promise<string>} AI response
 */
export const chatWithIntel = async (message, conversationHistory = []) => {
